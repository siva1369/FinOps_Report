...
with Live(table, console=console, refresh_per_second=1):
    for profile in profiles:
        ...
        ec2_data = ec2_summary(session)

        cost_by_service_str = '\n'.join([
            f"{group['Keys'][0]}: ${float(group['Metrics']['UnblendedCost']['Amount']):.2f}"
            for group in cost_data['current_month_cost_by_service']
        ])

        budget_status_str = '\n'.join([
            f"{budget['name']} limit: ${budget['limit']:.2f}\n{budget['name']} actual: ${budget['actual']:.2f}"
            for budget in cost_data['budgets'] if 'error' not in budget
        ]) or "No Budgets Found"

        ec2_summary_str = f"Running: {ec2_data['running']}\nStopped: {ec2_data['stopped']}"

        table.add_row(
            f"[bright_magenta]{profile}[/]",
            f"[bright_red]${cost_data['last_month']:.2f}[/]",
            f"[bright_green]${cost_data['current_month']:.2f}[/]",
            f"[bright_green]{cost_by_service_str}[/]",
            f"[bright_yellow]{budget_status_str}[/]",
            f"[bright_red]{ec2_summary_str}[/]"
        )
